#!/usr/bin/env python3
"""Compute CPDAG discovery quality metrics for CSuite and custom SCM results.

This script scans comparison experiment CSVs, extracts CPDAGs used in
`cpdag_discovered` runs, and computes discovery metrics against ground-truth DAGs.
...
def _find_result_csvs(repo_root: Path) -> list[Path]:
    ...
    # Canonical source for rebuttal-ready, cleaned comparison data.
    data_dir = repo_root / "causal_experiments" / "results" / "comparison_experiment" / "data"
    if data_dir.exists():
    ...
    custom_root = repo_root / "causal_experiments" / "custom_scm_experiment" / "comparison_experiment"
    ...
    if other_results.exists():
        candidates.extend(sorted(other_results.glob("**/*comparison_experiment*.csv")))
...
def _parse_dataset_id(path: Path) -> tuple[str, str]:
    ...
    if stem.startswith("result_csuite_"):
        # result_csuite_mixed_confounding_comparison_experiment_cleaned_reps_100
        match = re.search(r"result_(csuite_[^_]+(?:_[^_]+)*)_comparison_experiment", stem)
        if match:
...
def main() -> None:
    ...
    for path in csv_paths:
        ...
        # Inspect first valid graph for node count (custom SCM inference)
        n_nodes_hint = None
        for graph_value in df["graph_structure"].dropna().tolist():
        ...
        try:
            true_adj = _true_dag_for_dataset(dataset_id, source, repo_root, n_nodes_hint)
        except Exception:
