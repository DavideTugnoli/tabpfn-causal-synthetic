...
3. DAG-aware TabPFN with discovered REX constraints (dag_discovered)
...
Usage Examples:
    ...
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py
    
    # Run ALL algorithms in test mode (faster)
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --test
    
    # Run ONLY vanilla algorithm (all 3 orderings: original, topological, worst)
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --algorithm vanilla
    
    # Run ONLY vanilla algorithm in test mode
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --algorithm vanilla --test
    
    # Run ONLY vanilla with specific column ordering (useful for splitting heavy jobs)
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --algorithm vanilla --column-order original
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --algorithm vanilla --column-order topological
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --algorithm vanilla --column-order reverse_topological
    
    # Run ONLY DAG-aware algorithm (original ordering only)
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --algorithm dag
    
    # Run discovered DAG from precomputed REX graphs
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --algorithm dag_discovered
    
    # Other options work with --algorithm filter
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --algorithm dag_discovered --save-datasets
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --algorithm dag_discovered --save-every 25
    uv run causal_experiments/csuite_experiment/comparison_experiment_csuite/csuite_comparison_experiment_rex.py --datasets csuite_nonlin_simpson --algorithm dag_discovered
"""
...
_DEFAULT_REX_GRAPHS_ROOT = (
    Path("causal_experiments")
    / "csuite_experiment"
    / "comparison_experiment_csuite"
    / "rex_graphs"
    / "anm_allnpz_ts20_100_500"
    / "comparison"
)

_DEFAULT_SOURCE_SPLITS_ROOT = (
    Path("causal_experiments")
    / "csuite_experiment"
    / "comparison_experiment_csuite"
    / "results"
...
def _index_rex_graphs(
    rex_graphs_root: Path,
    dataset_name: str,
) -> dict[tuple[int, int], Path]:
    """Index REX JSON graphs by (train_size, seed)."""
    dataset_dir = rex_graphs_root / dataset_name
    if not dataset_dir.exists():
        return {}

    index: dict[tuple[int, int], Path] = {}
    pattern = re.compile(r"^ts(\d+)_s(\d+)$")
    for run_dir in sorted(dataset_dir.iterdir()):
        ...
        graph_path = run_dir / "rex_union.json"
        if graph_path.exists():
            index[(train_size, seed)] = graph_path
    return index


def _load_rex_dag_from_json(
    graph_path: Path,
    expected_column_names: list[str],
) -> dict[int, list[int]]:
    """Load REX DAG JSON and convert to TabPFN DAG format {child: [parents]}."""
    with graph_path.open("r", encoding="utf-8") as handle:
        payload = json.load(handle)

    algo_meta = payload.get("algorithm", {})
    if algo_meta.get("graph_type") != "dag":
        raise ValueError(f"Expected DAG graph_type in {graph_path}")

    payload_columns = payload.get("column_names")
    if payload_columns is not None and list(payload_columns) != expected_column_names:
        raise ValueError(
            f"Column mismatch for {graph_path}: expected {expected_column_names}, got {payload_columns}"
        )

    n_features = len(expected_column_names)
    dag: dict[int, list[int]] = {idx: [] for idx in range(n_features)}
    edges_idx = payload.get("edges_idx", [])
    if not isinstance(edges_idx, list):
        raise ValueError(f"Invalid edges_idx in {graph_path}")

    for edge in edges_idx:
        if not isinstance(edge, list) or len(edge) != 2:
            raise ValueError(f"Invalid edge {edge!r} in {graph_path}")
        parent = int(edge[0])
        child = int(edge[1])
        if not (0 <= parent < n_features and 0 <= child < n_features):
            raise ValueError(f"Out-of-range edge {edge!r} in {graph_path}")
        if parent != child:
            dag[child].append(parent)

    for child in dag:
        dag[child] = sorted(set(dag[child]))

    # Validate acyclicity
    from graphlib import TopologicalSorter
    _ = list(TopologicalSorter(dag).static_order())
    return dag
...
def run_single_experiment(
    ...
    try:
        ...
        if column_ordering_used is not None and (
            algorithm == "vanilla"
            or algorithm == "dag"
            or algorithm == "dag_discovered"
            or algorithm.startswith("cpdag_")
        ):
...
def main(
    ...
    dataset_name: str = "csuite_large_backdoor",
    ...
    train_sizes_list: list[int] | None = None,
    rex_graphs_root: str | None = None,
    source_splits_root: str | None = None,
    ...
    Args:
        ...
        skip_seeds: Explicit seed values to skip
        rex_graphs_root: Root directory for copied REX DAG JSON files
        source_splits_root: Directory root containing pre-existing train/test NPZ splits
        results_root: Output root for this REX run
    """
    # ======================
    ...
    if train_sizes_list:
        invalid_train_sizes = sorted(set(train_sizes_list) - set(TRAIN_SIZES))
        if invalid_train_sizes:
            raise ValueError(f"Unsupported explicit train sizes: {invalid_train_sizes}")
        TRAIN_SIZES = list(train_sizes_list)
    ...
    if explicit_seeds is not None:
        if len(explicit_seeds) != total_datasets_needed:
            raise ValueError(
                f"Expected {total_datasets_needed} explicit seeds, got {len(explicit_seeds)}"
            )
        selected_seeds = list(explicit_seeds)
        if len(set(selected_seeds)) != len(selected_seeds):
            raise ValueError("Explicit seeds must be unique")
        print(f"  Using explicit recovery seed schedule: {selected_seeds}")
    else:
    ...
    if dataset_name not in _ANM_DATASETS:
        raise ValueError(
            f"Dataset '{dataset_name}' is not ANM. Supported: {sorted(_ANM_DATASETS)}"
        )

    rex_graphs_base = (
        Path(rex_graphs_root).resolve()
        if rex_graphs_root is not None
        else _DEFAULT_REX_GRAPHS_ROOT.resolve()
    )
    source_splits_base = (
        Path(source_splits_root).resolve()
        if source_splits_root is not None
        else _DEFAULT_SOURCE_SPLITS_ROOT.resolve()
    )
    results_base = (
        Path(results_root).resolve()
        if results_root is not None
        else _DEFAULT_RESULTS_ROOT.resolve()
    )

    print(f"REX graphs root: {rex_graphs_base}")
    print(f"Source splits root: {source_splits_base}")
    print(f"Results root: {results_base}")

    # Output configuration (separate root to avoid mixing with old runs)
    output_dir = results_base / dataset_name
    ...
    source_dataset_dir = source_splits_base / dataset_name / "datasets"
    if not source_dataset_dir.exists():
        raise ValueError(
            f"Source split directory not found: {source_dataset_dir}"
        )

    source_global_test_path = source_dataset_dir / "global_test_set.npz"
    if not source_global_test_path.exists():
        raise ValueError(
            f"Missing source global test set: {source_global_test_path}"
        )
    with np.load(str(source_global_test_path), allow_pickle=True) as source_test_npz:
        loaded_test = source_test_npz["X_test"]
        loaded_cols = source_test_npz["column_names"].tolist()
    if list(loaded_cols) != column_names or loaded_test.shape[1] != len(column_names):
        raise ValueError(
            "Source global test set schema mismatch with CSuite dataset metadata."
        )
    global_test_set = loaded_test
    print(f" Loaded global test set from source splits: {source_global_test_path}")
    ...
    rex_graphs_index = _index_rex_graphs(rex_graphs_base, dataset_name)
    if not rex_graphs_index:
        raise ValueError(
            f"No REX DAG graphs found for dataset '{dataset_name}' in {rex_graphs_base}"
        )
    by_ts: dict[int, int] = {}
    for (train_size, _seed), _path in rex_graphs_index.items():
        by_ts[train_size] = by_ts.get(train_size, 0) + 1
    print("  - REX DAG coverage:")
    for ts in sorted(by_ts):
        print(f"    ts={ts}: {by_ts[ts]} graphs")
    ...
    configs = {
        ...
        # Discovered DAG from precomputed REX graphs (replaces PC discovered CPDAG)
        "dag_discovered": {
            "orderings": [
                ("topological", orderings_dict["topological"])
            ],
    ...
    # Keep the original train/test splits untouched when source and output coincide.
    source_dataset_root = (source_splits_base / dataset_name).resolve()
    if output_dir.resolve() != source_dataset_root:
        save_global_test_set(global_test_set, output_dir, column_names)
    else:
        print(" Reusing in-place global test set from source splits (no rewrite).")
    ...
    for train_size in TRAIN_SIZES:
        for rep_idx in range(N_REPETITIONS):
            ...
            dataset_file = source_dataset_dir / f"train_ts{train_size}_s{seed}.npz"
            loaded_successfully = False

            if not dataset_file.exists():
                raise ValueError(
                    f"Missing pre-generated train split: {dataset_file}"
                )
            ...
            try:
                with np.load(str(dataset_file), allow_pickle=True) as data:
                    ...
                    if list(cols_loaded) != column_names or X_loaded.shape[1] != len(column_names):
                        raise ValueError(
                            f"Schema mismatch in source split {dataset_file.name}"
                        )
                ...
                raise RuntimeError(f"Failed loading source split {dataset_file}: {exc}") from exc
            ...
            for train_size in TRAIN_SIZES:
                for rep_idx in range(N_REPETITIONS):
                    ...
                    try:
                        ...
                        cpdag_to_use = cpdag if algorithm.startswith("cpdag_") else None
                        dag_discovered = None
                        rex_graph_path = None
                        if algorithm == "dag_discovered":
                            rex_graph_path = rex_graphs_index.get((train_size, seed))
                            if rex_graph_path is None:
                                print(
                                    f"  → Missing REX DAG for train_size={train_size}, seed={seed}; skipping."
                                )
                                continue
                            dag_discovered = _load_rex_dag_from_json(
                                rex_graph_path, column_names
                            )
                        ...
                        elif algorithm == "dag" or algorithm == "dag_discovered":
                            dag_to_use = dag_discovered if algorithm == "dag_discovered" else dag
                            if dag_to_use is None:
                                raise ValueError(f"DAG is required for {algorithm} algorithm but got None")
                            # Keep the original pipeline, but when using discovered DAGs
                            # the topological ordering must be computed from the discovered DAG itself.
                            ordering_dag = dag_to_use if algorithm == "dag_discovered" else dag_dict
                            ...
                            if (column_order != "original") or (column_order == "original" and random_original_ordering is not None):
                                ...
                                if column_order == "original" and random_original_ordering is not None:
                                    ...
                                    X_train_reordered, column_ordering_used, updated_categorical_features = prepare_vanilla_data(
                                        X_train_original, column_order, ordering_dag, column_names, categorical_indices
                                    )
                                # Remap DAG indices to the new column order
                                old_to_new = {old_idx: new_idx for new_idx, old_idx in enumerate(column_ordering_used)}
                                dag_reordered = {}
                                for old_child, old_parents in dag_to_use.items():
                                ...
                                X_train_prepared, dag_prepared = prepare_dag_data(
                                    X_train_original, dag_to_use, column_names
                                )
                        ...
                        else:  # algorithm == "dag" or "dag_discovered"
                            graph_dict = dag_discovered if algorithm == "dag_discovered" else dag
                        ...
                        if algorithm == "dag_discovered" and rex_graph_path is not None:
                            result_row["rex_graph_path"] = str(rex_graph_path)
                        ...
                        if save_synthetic:
                            synthetic_algorithm_name = (
                                "dag_rex" if algorithm == "dag_discovered" else algorithm
                            )
                            synthetic_path = save_synthetic_data(
                                synthetic_df=synthetic_df,
                                algorithm=synthetic_algorithm_name,
                                column_order=column_order,
                                train_size=train_size,
                                seed=seed,
                                output_dir=output_dir,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--datasets", type=str, default="csuite_large_backdoor", help="Name of CSuite dataset to use (ANM datasets only)")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Use exactly these seeds in schedule order (isolated recovery runs only).",
    )
    parser.add_argument(
        "--train-sizes-list",
        type=int,
        nargs="+",
        default=None,
        help="Restrict the run to these explicit training sizes.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--algorithm",
        help=(
            "Run only specific algorithm: vanilla, dag, dag_discovered, cpdag_minimal."
        ),
    )
    parser.add_argument(
        "--column-order",
        help=(
            "Run only specific column ordering (vanilla/DAG): original, topological, reverse_topological"
        ),
    )
    parser.add_argument(
        "--rex-graphs-root",
        type=str,
        default=None,
        help=f"Root directory for copied REX graphs (default: {_DEFAULT_REX_GRAPHS_ROOT})",
    )
    parser.add_argument(
        "--source-splits-root",
        type=str,
        default=None,
        help=f"Root directory for existing CSuite train/test splits (default: {_DEFAULT_SOURCE_SPLITS_ROOT})",
    )
    parser.add_argument(
        "--results-root",
        type=str,
        default=None,
        help=f"Output root for this REX run (default: {_DEFAULT_RESULTS_ROOT})",
    )
    ...
    main(
        ...
        train_sizes_list=args.train_sizes_list,
        rex_graphs_root=args.rex_graphs_root,
        source_splits_root=args.source_splits_root,
