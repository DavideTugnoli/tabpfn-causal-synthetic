...
class GenerateSyntheticDataExperiment(Experiment):
    ...
    def run(self, tabpfn, **kwargs):
        ...
        Args:
            tabpfn: TabPFN unsupervised model
            **kwargs: Experiment parameters including:
                X: Input data tensor
                y: Target data (optional, not used in unsupervised)  
                ...
                n_samples: Number of synthetic samples to generate (default: X.shape[0])
                n_permutations: Number of permutations for generation (default: 3)
                dag: Optional DAG dictionary for causal generation
                cpdag: Optional CPDAG dictionary/matrix for hybrid causal generation
                causal_structures_last: Optional bool to generate causal structures after correlational ones
        
        Returns:
            None (results stored in self.data, self.synthetic_X, etc.)
        """
        with warnings.catch_warnings():
            ...
            causal_structures_last = kwargs.get("causal_structures_last", False)

            self.X, self.y = X, y
            self.X = self.X[:, indices]
            old_features_names = attribute_names
            # Use attribute_names in the SAME order as the CURRENT X columns
            # By passing attribute_names already aligned to current order from the caller,
            # indices is typically identity; but keep the mapping for safety.
            self.feature_names = [attribute_names[i] for i in indices]
            # generate subset of categorical indices
            provided_categorical = kwargs.get("categorical_features")
            if provided_categorical is None:
                categorical_features = list(getattr(tabpfn, "categorical_features", []))
            else:
                categorical_features = list(provided_categorical)
            if hasattr(tabpfn, "set_categorical_features"):
                tabpfn.set_categorical_features(categorical_features)
            # Provide feature names (in CURRENT order) for accurate debug logs downstream
            if hasattr(tabpfn, "set_feature_names"):
                tabpfn.set_feature_names(self.feature_names)
            tabpfn.fit(self.X)
            ...
            self.plot()
            # Close figures aggressively to avoid leaking thousands of PairGrid plots across long runs
            plt.close("all")
